...
class BoneMesh:
    def __init__(self, name: str, path: str = None, mesh: trimesh.Trimesh = None):
        self.name = name
        self.mesh = self.__read_stl(path) if path else mesh
        self.x_bounds, self.y_bounds, self.z_bounds = self.__get_bounds()
        self.xyz_size = np.abs(
            np.array(
                [
        ...
        self.is_tibia = self.__is_tibia()
        return None

    def __is_tibia(self):
        return self.name.split()[-1].lower() == "tibia"
...
class DicomCT:
    def __init__(self, dicomdir_path: str):
        ...
        self.axial_slices = self.__get_planar_slices(image_paths, [1, 0, 0, 0, 1, 0])
        self.img3d = np.array([slice.pixel_array for slice in self.axial_slices])
        self.ni, self.nj, self.nk = self.axial_slices[0].pixel_array.shape + (
            len(self.axial_slices),
        )
    ...
    def __get_image_paths_from_dicomdir(self, dicom_path: str):
        ...
        for record in study.children:
            ...
            images = [ii for ii in record.children if ii.DirectoryRecordType == "IMAGE"]
    ...
    def __get_planar_slices(
        self, image_paths: list, plane_orientation: list[int, int, int, int, int, int]
    ) -> list[pydicom.FileDataset]:
        slices = []
        filtered_paths = [
            path
            for path in image_paths
            if pydicom.dcmread(path, stop_before_pixels=True).ImageOrientationPatient
            == plane_orientation
        ]
    ...
    def __get_bounding_box(self):
        ...
        max_x, max_y = min_x + (self.ni - 1) * self.dx, min_y + (self.nj - 1) * self.dy
        min_z, max_z = (
            self.axial_slices[0].SliceLocation,
            self.axial_slices[-1].SliceLocation,
        )
    ...
    def get_k_image(self, k: int, callibrate: bool = False):
        ...
        hu_img = (
            apply_modality_lut(raw_img, self.axial_slices[k]) if callibrate else raw_img
        )
...
class BoneCT:
    ...
    def __get_slices(self):
        ...
        for k, slice in enumerate(self.body_ct.axial_slices):
            ...
            if self.bone_mesh.z_bounds[0] <= z <= self.bone_mesh.z_bounds[1]:
                ...
                bone_section_poly_points = self.bone_mesh.get_z_section_points(z)
                ...
                bone_section_img = geom.get_poly_image(
                    bone_section_poly_pixels, self.body_ct.ni, self.body_ct.nj
                )
    ...
    def __get_all_points_4d(self):
        ...
        x = x_bounds[0] + (x_bounds[1] - x_bounds[0]) * i / self.img_3d.shape[2]
        y = y_bounds[1] + (y_bounds[0] - y_bounds[1]) * j / self.img_3d.shape[1]
        z = z_bounds[0] + (z_bounds[1] - z_bounds[0]) * k / self.img_3d.shape[0]
    ...
    def __get_medial_points_4d(self):
        ...
        mask = (
            ...
            if self.side == "left"
            else (self.all_points_4d[:, 0] > self.bone_mesh.x_mid_plane)
        )
    ...
    def __get_implant_roi_points_4d(self, filter_percent):
        ...
        top_most_points = self.__get_top_most_projected_points(
            projected_points_3d, filter_percent
        )
