...
def filter_points_by_desired_peg_roi_vol(
    ...
    return (
        ...
        filter_2_labels,
        min_density_thresh,
        thresholded_point_cloud,
    )


def tibia_analysis(
    bone_mesh, bone_CT, init_density_percentile_thresh, desired_peg_vol_ratio
):
    ...
    (
        ...
        min_density_thresh,
        thresholded_point_cloud,
    ) = filter_points_by_desired_peg_roi_vol(
        implant_roi_cn_points, init_density_percentile_thresh, desired_peg_vol_ratio
    )
    peg_cn_hull_analyser = msa.ConvexHullAnalyser(filter_2_points)
    (
        ...
        hull_centres,
        point_centers,
    ) = peg_cn_hull_analyser.sliced_convex_hull_2d()
    peg_cn_hull_mesh = geom.convex_hull_to_trimesh(peg_cn_hull_analyser.convex_hull_3d)
    ...
    peg_un_cn_hull_analyser = msa.ConvexHullAnalyser(filter_2_points_uncentered)
    ...
    peg_un_cn_cylinder_mesh = geom.create_cylinder_from_trimesh(peg_un_cn_hull_mesh)
    print(f"Filter 2 Convex Hull Volume: {peg_un_cn_hull_volume} mm^3")

    # Plots:
    medial_points_plot = msv.PointCloudPlot(
        medial_points,
        normalised=False,
    ...
    implant_roi_points_plot = msv.PointCloudPlot(
        implant_roi_points,
        normalised=False,
    ...
    centred_nomalised_points_plot = msv.PointCloudPlot(
    ...
    filter_1_plot = msv.PointCloudPlot(
        filter_1_points,
        normalised=True,
    ...
    filter_2_clusters_plot = msv.PredictedClustersPlot(filter_1_points, filter_2_labels)
    filter_2_clusters_plot.show()
    filter_2_clusters_plot.close()

    filter_2_plot = msv.PointCloudPlot(
        filter_2_points,
        normalised=True,
    ...
    convex_hull_2d_plot = msv.PointCloudWithPolygonsPlot(
        filter_2_points,
        convex_hull_2d_vertices_by_z,
        other_lines=[hull_centres, point_centers],
        title=f"{bone_mesh.name} Peg ROI - 2D Sliced Convex Hull",
    )
    convex_hull_2d_plot.show()
    convex_hull_2d_plot.close()

    convex_hull_3d_plot = msv.GiftWrapPlot(peg_cn_hull_mesh, filter_2_points)
    convex_hull_3d_plot.plot()
    convex_hull_3d_plot.close()

    overlay_mesh_plot = msv.TriMeshPlot(
        [peg_un_cn_cylinder_mesh, peg_un_cn_hull_mesh],
        title=f"{bone_mesh.name} Peg ROI - Comparison of Convex Hull and Cylinder Fit",
    )
    overlay_mesh_plot.show()
    overlay_mesh_plot.close()

    roi_visualiser = msv.RoiVisualiser(
        bone_mesh.mesh,
        peg_un_cn_hull_mesh,
    ...
    roi_visualiser_cylinder = msv.RoiVisualiser(
        bone_mesh.mesh,
...
def per_bone_analysis(body_CT, bone_mesh):
    ...
    bone_plot = msv.PointCloudPlot(
        bone_CT.all_points_4d,
        normalised=False,
...
def pre_analysis_plots(body_CT, bone_meshes):
    ...
    # Plot the bone STLS:
    mesh_plot = msv.Bone3DPlot(bone_meshes, colors=bone_colors, title="STL Bone Meshes")
    mesh_plot.show()
    mesh_plot.close()

    # Plot the CT slices with the segmented bone regions:
    segemented_region_slider_plot = msv.SegmentedRegionSliderPlot(
        body_CT, bone_meshes, callibrate=True, colors=bone_colors
    )
